...
test("Eliscript core modules compile and execute against runtime protocols", async () => {
  ...
  try {
    ...
    expect(api.report).toMatchObject({
      ...
      "vector-indexed": true,
      "vector-seqable": true,
      "vector-reducible": true,
      "vector-reversible": true,
      "vector-associative": true,
      "vector-collection": true,
      "vector-vector": true,
      "vector-sequential": true,
      "vector-sequence": false,
      "list-list": true,
      "list-sequence": true,
      "map-map": true,
      "set-set": true,
      "native-array-collection": false,
      "bounded-vector-count": 2,
      "distinct-scalars": true,
      "distinct-vectors": false,
      "empty-vector-empty": true,
      "non-empty-vector-same": true,
      "empty-vector-absent": true,
      "vector-second": 2,
      "vector-has-two": true,
      "vector-seq-count": 3,
      "map-left": 10,
      "map-associative": true,
      "map-indexed": false,
    ...
    const nodeCheck = [
      ...
      "if (!api.report['vector-sequential'] || api.report['vector-sequence']) process.exit(1);",
      "if (!api.report['list-list'] || !api.report['list-sequence']) process.exit(1);",
      "if (!api.report['map-map'] || !api.report['set-set']) process.exit(1);",
