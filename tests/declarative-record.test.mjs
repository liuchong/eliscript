...
const compilationUnits = [
  ...
  ["stdlib/core/record.eli", "stdlib/core/record.eli"],
  ["tests/fixtures/declarative-record.eli",
    "tests/fixtures/declarative-record.mjs"],
];
const hostFixture = resolve(root, "tests/fixtures/declarative-record-host.mjs");
...
test("defrecord is identical across seed and self-hosted compilers", async () => {
  const directory = await mkdtemp(resolve(tmpdir(), "eliscript-defrecord-"));
  ...
  try {
    ...
    expect(reports[0]).toEqual({
      identity: { type: "Person", record: true, predicate: true },
      construction: {
        name: "Ada",
        age: 36,
        mappedName: "Grace",
      ...
      map: {
        count: 3,
        role: "mathematician",
        extraRemovalKeepsRecord: true,
        fieldRemovalKeepsRecord: false,
        fieldRemovalCount: 2,
      },
      value: { equal: true, hashEqual: true },
      protocol: "Ada:36",
    });
  } finally {
