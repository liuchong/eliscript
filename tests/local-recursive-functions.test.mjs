...
const source = resolve(root, "tests/fixtures/local-recursive-functions.eli");
const output = "tests/fixtures/local-recursive-functions.mjs";
const hostFixture = resolve(
  root,
  "tests/fixtures/local-recursive-functions-host.mjs",
);
...
test("local recursive functions agree across compilers and local hosts", async () => {
  const directory = await mkdtemp(resolve(tmpdir(), "eliscript-letfn-"));
