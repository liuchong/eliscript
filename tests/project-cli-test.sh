...
OUTPUT=$(
  ...
  "$PROJECT_DIR/bin/eliscript-build" \
    ...
    "$PROJECT_DIR/examples/sequence-cli/main.eli"
)

ENTRY="$TMP_DIR/build/examples/sequence-cli/main.mjs"
...
test "$RESULT" = '{"values":[1,2,3,4,5,6,7],"squares":[1,4,9,16,25,36,49],"even":[2,4,6],"sum":28}'

if "$PROJECT_DIR/bin/eliscript-build" "$PROJECT_DIR/examples/sequence-cli/main.eli" \
