...
- `:keys` accepts one vector of unqualified symbols and looks up the keyword
  with the same name.
...
the core collection lookup protocol. For plain JavaScript objects, keyword keys
map to their qualified string names; persistent maps retain keyword value keys.
...
Compilation rejects duplicate directives, non-vector `:keys`, qualified or
marker names in `:keys`, non-map `:or`, defaults without a matching scalar
target, invalid `:as` values, unknown directives, and unsupported explicit
lookup keys.
