...
weakened commands, and workflow drift. At the historical M7 gate, remote matrix
results were required; specification 0179 later replaced that final-acceptance
boundary with an explicit direct-local required profile. See
...
required Emacs 29/30 profile, local onboarding exercise, and M10 exit audit are
now complete; optional Linux target evidence remains separate.
...
The full required suite passes on every system in the explicit local acceptance
profile, with Emacs 29 and 30, the declared Bun version, and Node.js where
applicable. Exact versions are recorded in the acceptance manifest. Linux
remains a maintained optional target under specification 0179 and does not
block the current final gate.
