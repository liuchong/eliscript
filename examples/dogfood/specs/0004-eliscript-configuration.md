...
## Visibility And Private Input

A remote source declares `visibility` as `public` or `private`. A private source
publishes into public output, so it is accepted only when the configuration
acknowledges that with `publishing.private-input :acknowledged`. A private
source may also only use static snapshots: a live or hybrid channel is read by
the reader's browser, which holds no credential for a repository it cannot see,
so those modes fail validation instead of rendering a channel that cannot load.
